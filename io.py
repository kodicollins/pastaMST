...
class IO():
  ...
  def define_parser(self):
    ...
    parseargs.add_argument('-o', '--output', dest = 'outdir', require = False, default = 'opal',
      help = "Full path to directory where OPAL alignments are stored. Default: opal")
    parseargs.add_argument('-j', '--opal', dest = 'opal', required = False, default = 'opal.jar',
      help = "Full path to OPAL jar. Default: opal.jar")
    parseargs.add_argument('-f', '--final', dest = 'final', required = False, default = 'alignedMST.fasta',
      help = "Full path to output final alignment of PASTA after using MST of sub-alignments. Default = alignedMST.fasta")
    parseargs.add_argument('-m', '--max', dest = 'MAX', action = 'store_true', default = False, required = False,
      help = "Flag for making the algorithm a maximum spanning tree instead of minimum spanning tree. Default: False")
    parseargs.add_argument('-x', '--method', dest = 'method', required = False, default = 'median',
      help = "Method for scoring alignment. Options = {median, mean, max, min}, Default: median")
    parseargs.add_argument('-c', '--consec', dest = 'consec', required = False, action = 'store_true', default = False,
      help = "Flag for changing from overall number of indels to longest consecutive indel. Default: False")
    parseargs.add_argument('-t', '--step', dest = 'step', required = False, default = '2',
      help = "PASTA step to align using MST. Default: 2 (min: 0, max: 2 if using default PASTA)")
    parseargs.add_argument('-s', '--split', dest = 'split', required = False, default = 'centroid',
